...
class Wishart():
    ...
    def to(self, device):
        ...
        if hasattr(self, 'invU_0') and self.invU_0 is not None:
            self.invU_0 = self.invU_0.to(device)
        ...
        if hasattr(self, 'nu_0') and self.nu_0 is not None:
            self.nu_0 = self.nu_0.to(device)
    ...
    def log_mvgamma(self,nu):
        return (nu.unsqueeze(-1) - torch.arange(self.dim, device=nu.device)/2.0).lgamma().sum(-1)

    def log_mvdigamma(self,nu):
        return (nu.unsqueeze(-1) - torch.arange(self.dim, device=nu.device)/2.0).digamma().sum(-1)
    ...
    def ElogdetinvSigma(self):
        return self.dim*np.log(2) - self.logdet_invU + ((self.nu.unsqueeze(-1) - torch.arange(self.dim, device=self.nu.device))/2.0).digamma().sum(-1)
