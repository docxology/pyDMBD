...
class LinearDynamicalSystems(torch.nn.Module):
...
#        invGamma, invGammamu, Residual, logZ_b[-1] = self.backward_step_with_Residual(invGamma, invGammamu, Residual, invSigma_like[0], invSigmamu_like[0],Residual_like[0],u[0])
    ...
    def to(self, device):
        ...
        This is necessary to ensure all tensors, including distribution objects, are on the same device.
        
        Args:
            device: The device to move the model to
            
        Returns:
            self: The model on the specified device
        """
        # Call the parent class to() method
        super_model = super().to(device)
        
        # Move distribution objects to the device
        if hasattr(self, 'x0'):
        ...
        if hasattr(self, 'obs_model'):
            self.obs_model = self.obs_model.to(device)
        
        # Move any tensor attributes to the device
        if hasattr(self, 'invQ') and self.invQ is not None:
            self.invQ = self.invQ.to(device)
        
        if hasattr(self, 'QA_xp_x') and self.QA_xp_x is not None:
            self.QA_xp_x = self.QA_xp_x.to(device)
        
        if hasattr(self, 'px') and self.px is not None:
