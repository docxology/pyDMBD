...
#        self.transition = Dirichlet(0.5*torch.ones(self.batch_shape+(self.hidden_dim,self.hidden_dim),requires_grad=False)).to_event(1)
    ...
    def to(self, device):
        """
        Move the HMM to the specified device.
        
        Args:
            device: The device to move the distribution to.
            
        Returns:
            The HMM on the specified device.
        """
        # Move the observation distribution to the device
        if hasattr(self, 'obs_dist') and self.obs_dist is not None:
            if hasattr(self.obs_dist, 'to'):
                self.obs_dist.to(device)
        
        # Move the transition distribution to the device
        if hasattr(self, 'transition') and self.transition is not None:
            if hasattr(self.transition, 'to'):
                self.transition.to(device)
        
        # Move the initial distribution to the device
        if hasattr(self, 'initial') and self.initial is not None:
            if hasattr(self.initial, 'to'):
                self.initial.to(device)
        
        # Move the transition mask to the device if it exists
        if hasattr(self, 'transition_mask') and self.transition_mask is not None:
            self.transition_mask = self.transition_mask.to(device)
        
        # Move the posterior probabilities to the device if they exist
        if hasattr(self, 'p') and self.p is not None:
            self.p = self.p.to(device)
        
        # Move sufficient statistics to the device if they exist
        ...
        if hasattr(self, 'NA') and self.NA is not None:
